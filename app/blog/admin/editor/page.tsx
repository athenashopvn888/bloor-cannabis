...
function BlogEditorInner() {
  ...
  const [author, setAuthor] = useState("Bloor Cannabis Dispensary Team");
  ...
  useEffect(() => {
    ...
    fetch(`${APPS_SCRIPT_URL}?action=blog&store=BCD01&admin=1`)
      .then((r) => r.json())
      .then((data) => {
        ...
        if (post) {
          ...
          setAuthor(post.author || "Bloor Cannabis Dispensary Team");
  ...
  async function handleSave() {
    ...
    const payload = editId
      ...
      : { action: "create", title, content, author, published, store: "BCD01" };
